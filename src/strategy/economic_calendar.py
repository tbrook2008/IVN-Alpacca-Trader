...
class EconomicCalendar:
    def __init__(self):
        ...
        self.cache_file = os.path.join(base_dir, "..", "..", "economic_cache.json")
    ...
    def fetch_events(self):
        ...
        if os.path.exists(self.cache_file):
            try:
                ...
                if (datetime.now(eastern) - cache_time).total_seconds() < 86400:
                    ...
                    logger.info(f"Loaded {len(self.high_impact_events)} High Impact events from local cache.")
                    return
            except Exception as e:
                logger.error(f"Error reading economic cache: {e}")
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Error saving economic cache: {e}")
                
            logger.info(f"Loaded {len(self.high_impact_events)} High Impact USD events from ForexFactory.")
    ...
    def is_news_blackout(self):
        """
        Returns True if current time is within 10 minutes before 
